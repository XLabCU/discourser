...
from .baseline_analysis_engine import BaselineAnalysisEngine
...
class AnalysisCoordinator:
    ...
    def __init__(self):
        ...
        self.baseline_engine = BaselineAnalysisEngine()
        self.baseline_results = {}
        self.baseline_analysis_ready = False
        ...
        self.combined_suggested_terms = None  
    ...
    def create_custom_vector_from_corpus(self, vector_name: str, positive_terms: List[str], 
        ...
        # The embedding engine will use the global model context
    ...
    def run_enhanced_topic_modeling(self, corpus_data: Dict, config: Dict) -> Tuple[bool, Dict, str]:
        """Delegate topic modeling to the vector analysis engine"""
            ...
            for i, (text, topic_id, prob_dist) in enumerate(zip(target_texts, target_topics, target_probabilities)):
                ...
                if i < len(self.target_embeddings):
                    ...
                    if most_similar_core_idx < len(self.core_metadata['paragraph_metadata']):
                        ...
                        # Initialize core_text
                        core_text = ""
                            # Get the actual text from the core corpus
                        filename = core_meta['filename']
                        para_idx = core_meta['paragraph_index']
                        ...
                        most_similar_core = {
                            ...
                            'text_preview': core_text[:200] + "..." if len(core_text) > 200 else core_text
                        }
    ...
    def generate_baseline_analysis(self, progress_callback=None) -> Tuple[bool, str]:
        """Generate baseline distributions for statistical significance testing"""
        try:
            if self.core_embeddings is None or self.target_embeddings is None:
                return False, "Both core and target corpora must be processed first"
            
            if progress_callback:
                progress_callback("Generating document shuffle baseline (this may take a few minutes)...")
            
            # Generate document shuffle baseline
            doc_baseline = self.baseline_engine.generate_document_shuffle_baseline(
                self.core_embeddings, 
                self.target_embeddings,
                n_permutations=1000,
                cache_key="main_document_analysis"
            )
            
            self.baseline_results['document_shuffle'] = doc_baseline
            
            if progress_callback:
                progress_callback("Baseline analysis complete!")
            
            self.baseline_analysis_ready = True
            return True, f"Baseline analysis complete. Generated {doc_baseline['n_permutations']} permutations."
            
        except Exception as e:
            error_msg = f"Error generating baseline analysis: {str(e)}"
            logger.error(error_msg)
            return False, error_msg

    def assess_similarity_significance(self, observed_value: float, 
                                     analysis_type: str = "document_shuffle") -> Dict:
        """Assess statistical significance of observed similarity against baseline"""
        if not self.baseline_analysis_ready or analysis_type not in self.baseline_results:
            return {'interpretation': 'Baseline analysis not available'}
        
        baseline_data = self.baseline_results[analysis_type]
        return self.baseline_engine.assess_significance(
            observed_value, 
            baseline_data['baseline_distribution'],
            test_type='greater'
        )
    ...
    def create_topic_network_visualization(self, target_similarities: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            for edge in target_similarities['network_edges']:
                if edge['weight'] > 0.75:  # Only show strong connections
                    G.add_edge(
                        edge['source'], 
                        edge['target'], 
                        weight=edge['weight'],
